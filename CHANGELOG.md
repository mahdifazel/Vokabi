...
- **Richer adjective details.** The adjective section now matches the verb section's depth: an example sentence with playback, the comparison card, a "Common combinations" list (correctly declined phrases like "ein schnelles Auto" with translations and per-phrase playback), and a Grammar card with the opposite and CEFR level. Curated data covers ~55 frequent A1/A2 adjectives; others keep the derived comparison and AI example. Non-gradable adjectives (leer, fertig) now show their details without a comparison table instead of nothing

- **Noun declension and gender hints.** Nouns get a "Declension" section on the word detail page: all four cases for singular and plural (using the enriched plural form), driven by a new on-device engine that knows the genitive rules (des Hundes, des Ergebnisses, des Kaktus), weak n-nouns (den Jungen, des Herrn, des Namens, plus an "n-noun" badge), and the dative plural -n. When the noun's ending matches a reliable gender rule that agrees with its article, a "Why die?" hint explains it (nouns ending in -ung are feminine, -chen are neuter, ...)
