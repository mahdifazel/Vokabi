...
**Decision:** An `is_default` flag on `preset_groups`, toggled from the back office (star on the row, switch in the create form). Seeding stays client-side and reuses the copy semantics of decision 17: after a successful sync pull, `seedDefaultPresetGroups()` (`lib/words.ts`) fetches presets, and for every default whose id it hasn't processed before it creates a plain local group and runs the words through `addWordsFromText()`; the resulting group syncs like any user-created one. Processed preset ids are recorded per account in localStorage (`vokabi.seededPresets.<userId>`), and a preset whose name already exists locally is only marked as processed, never duplicated.
**Trade-offs:** Deleting a seeded default group sticks on the devices that recorded it, but a fresh device with an empty localStorage re-seeds it after its first pull (the deleted group no longer wins the name check), so the group can resurrect across the account. Accepted: no server-side per-user state, no schema changes to `words`/`groups`, and the sync engine stays untouched. Newly flagged defaults reach existing clients on their next app start (the fetch runs once per session). Rejected: a per-user "dismissed presets" table in Supabase (schema churn for an edge case) and undeletable server-driven groups (breaks the "user owns their data" model of decision 17).
