...
**Decision:** Both `/api/ai/*` routes run a provider chain (`extractWordsViaProviders` in `src/app/api/ai/_shared.ts`): Gemini first (default `gemini-flash-lite-latest`, Google's auto-updating flash-lite alias — concrete ids like `gemini-2.5-flash` 404 on API keys created after their retirement, and the plain flash tier turned into a slow frontier model with 3.5, measured at 30s+ per answer vs ~1-2s for flash-lite; one multimodal model for both the photo scan and OCR cleanup), Groq second on any Gemini failure (HTTP error, timeout, unparseable output, not configured), then the existing on-device fallbacks client-side. The Gemini key resolves as `app_settings.gemini_api_key` → `GEMINI_API_KEY`/`GOOGLE_API_KEY` env var, so the back office can override the env key without a redeploy; `/admin/settings` shows both providers as separate cards with their own save/test/remove, and the Gemini card flags when the env-var key is active. Per-provider timeouts are budgeted so the chain fits the client's request timeout (image: 15 s + 12 s inside 30 s; text: 12 s + 10 s inside 25 s). When both providers fail, the route answers 429 if either was rate limited (retry may help), else 502. Thinking is dialed down (`thinkingBudget: 0` on Gemini 2.5, `thinkingLevel: "minimal"` on 3.x and newer — the analog of Qwen's `reasoning_effort: "none"`) so reasoning tokens don't starve the JSON output or the latency budget.
