...
- **`scheduleDefinitionBackfill()`** (`lib/definitions.ts`) mirrors the example backfill for the **Meaning language** setting: while it is set to Deutsch, every ready word without a `definitionDe` gets a short A2-level German definition via `POST /api/ai/definitions` (batches of 10, same backoff/attempt/dictCache rules; sentence entries are skipped). The setting (`meaningLanguage: "en" | "de"`, default `"en"`, localStorage like all settings) switches every displayed meaning through `wordMeaning()` in `lib/words.ts` — word rows, word detail, mini player, flashcards, quiz building (`primaryMeaning()` in `lib/learn.ts`), lock-screen metadata, and the spoken translation (German definitions are spoken with the German voice) — falling back to the English translation until a word's definition arrives. `definitionDe` syncs as the `definition_de` column and is editable in the word detail edit sheet.
