...
2. **Push deletions** — outbox tombstones → `DELETE where uid in (…)`, then those specific tombstone ids are deleted (not `outbox.clear()`, which would swallow a deletion queued while the request was in flight and let the next pull resurrect that row)
3. **Push dirty groups**, then **dirty words** (local numeric `groupIds` are translated to group `uid`s for the wire; `favorite` 0/1 ↔ boolean). Afterwards `dirty` is cleared only on rows whose `updatedAt` still matches the pushed snapshot: background enrichment and the example/definition backfills write to these rows while the upsert is in flight, and marking those clean would drop the change from the next push *and* let the older remote copy win the merge in step 4
4. **Pull everything** — paged via `pullTable()`, keyset style on `uid`, until the server returns an empty page. A single `.select("*")` is **silently truncated** at the backend's `max-rows` (1000 by default on Supabase): no error, just a short result, which step 5 would read as "everything past row 1000 was deleted". Groups first (build uid→local-id map), then words; per row **last-write-wins**: local wins only if still dirty *and* newer
5. **Reconcile** — local non-dirty rows whose uid vanished remotely were deleted on another device → delete locally. Two guards must both hold per table, because "this uid wasn't in the pull" only means deletion when the pull is provably the complete set. (a) The pull must be `complete`, i.e. `pullTable()` paged all the way to an empty page rather than stopping at a row cap — without this, a library larger than `max-rows` loses every row past the cap on each sync and stays pinned to exactly that many words. (b) The result must not be empty for a table that had synced rows locally beforehand, which catches an untrustworthy pull (auth/session hiccup, network blip, momentary RLS mismatch) rather than proof everything was deleted — otherwise a single bad pull mass-deletes the whole local library in one `bulkDelete`. Either guard failing skips the delete for that table; a genuine remote deletion still lands on the next sane sync. Reconcile only ever deletes **locally** (no tombstone), so rows it removes by mistake are still in the cloud and come back once the pull is whole again.
