...
Two more self-healing passes run at the same trigger points (startup / after sync pulls):
...
- The `/api/ai/*` routes (`extract-words`, `extract-words-image`, `examples`) have no per-user rate limiting: any signed-in user can spend the AI quota. Groq's free tier is tight in practice (bursts of vision scans 429 within a minute), which is why Gemini is tried first and Groq only absorbs its failures; when both are rate limited the client shows a "scanner is busy" notice
