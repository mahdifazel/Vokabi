...
5. **Reconcile** — local non-dirty rows whose uid vanished remotely were deleted on another device → delete locally. Guarded per table: a pull that comes back completely empty for a table that had synced rows locally beforehand is treated as an untrustworthy pull (auth/session hiccup, network blip, momentary RLS mismatch) rather than proof everything was deleted, and skips the delete for that table — otherwise a single bad pull mass-deletes the whole local library in one `bulkDelete`. A genuine remote deletion still lands on the next successful sync.
