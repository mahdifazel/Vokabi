...
src/
  ...
  lib/
    ...
    definitions.ts         Resumable German-definition backfill (only while meaning language = Deutsch)
...
- Deletions and upserts are pushed **in chunks** (`DELETE_CHUNK`/`UPSERT_CHUNK` in `sync.ts`). PostgREST puts `uid=in.(…)` in the query string and starts returning 400 above ~24 KB of URL, i.e. around 650 uids: one bulk delete of that size failed the whole sync, and since its tombstones stay in the outbox it failed again on every retry, so sync stayed broken and nothing reached the cloud any more. Don't collapse these back into single requests.
- Reconcile writes every removal and every skipped cleanup to the `diag` log, so this class of bug shows up in Settings → hidden diagnostics (7 taps on the footer) instead of being silent. `e2e/sync-truncation.cloud.spec.ts` covers it against a scripted Supabase stub; run it with `npm run test:e2e -- --project=cloud-sync` after touching sync.
- The sync pull is **paged** (`pullTable` in `sync.ts`, keyset on `uid` until an empty page). A plain `supabase.from("words").select("*")` is silently truncated at the backend's `max-rows` (1000 by default on Supabase): the request succeeds, there is no error, the result is just short. Once a library passes that cap, every row beyond it looks deleted to the stale-row reconcile and gets `bulkDelete`d locally on every sync, pinning the device to exactly `max-rows` words while the rows sit untouched in the cloud (reconcile deletes locally only, never writes a tombstone). The symptom is newly added words and sentences vanishing shortly after they are added, since fresh and freshly updated rows are the ones the cap cuts off. Reconcile now runs only when `pullTable` reports `complete`. Don't replace the paging with a single select, and don't rely on the backend's row cap being large enough.
- The sync pull's stale-row reconcile (`syncNow` in `sync.ts`) only trusts an empty pull result for a table when that table had no synced rows locally beforehand (`syncedWordUids`/`syncedGroupUids`, captured before the pull). Those same sets are also the candidate list: a row is only ever deleted if this device already had it synced *before* the pull went out, so a row another tab pushed mid-request can't be judged by a server snapshot that predates it. And a deletion big enough to count as a purge (≥ 25 rows and ≥ 25% of the table) has to be confirmed by a second pull ≥ 30 s later before it is applied. Without that guard, any pull that comes back empty for a reason other than a real remote deletion — a transient auth/session hiccup, a network blip, a momentary RLS mismatch — reads as "everything was deleted elsewhere" and `bulkDelete`s the entire local library in one pass; the trigger is often innocuous, like adding a word (which just schedules the sync that then wipes the list). Don't remove this guard or make reconcile trust an empty/short result set unconditionally.
